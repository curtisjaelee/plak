...
exports.handler = async (event) => {
  ...
  try {
    ...
    const result = await client.query(
      `SELECT id, title, category, rank_position
       FROM activity
       WHERE couple_id = $1 AND bucket = $2 AND rank_position IS NOT NULL
    ...
    console.error(err);
    return { statusCode: 500, headers: corsHeaders, body: JSON.stringify({ error: 'Failed to fetch bucket activities' }) };
  }
};